...
SquModel::SquModel()
{
    m_WaveType=SQUARE_FUN;
    m_WaveNo=POSITIVE_SQUARE;
...
bool SquModel::SetWaveNo(uint8_t waveIndex)
{
    m_WaveNo=waveIndex;
    return true;
}

int SquModel::GetWaveNo()
{
    return m_WaveNo;
}

void SquModel::InitWaveNo(uint8_t waveIndex)
{
    m_WaveNo=waveIndex;
    m_SignalChannelOhm=LOW_OHM;
    m_Amp.Unit=QString("Vpk-pk");
...
bool SquModel::WaveParaSend()
{
    ...
    qDebug() << "value: " << GetAmpValue(m_Amp);
    qDebug() << "freq: "  << GetFreqValue(m_Frequence);
    return true;
}

void SquModel::WaveInit()
{
    InitWaveNo(POSITIVE_SQUARE);
    return;
}

bool SquModel::SetSignalChannelOhm(int ohm)
{
    m_SignalChannelOhm=ohm;
    return true;
}

int SquModel::GetSignalChannelOhm()
{
    return m_SignalChannelOhm;
}

bool SquModel::SetFrequencePara(Para freq)
{
    m_Frequence=freq;
    return true;
}

Para SquModel::GetFrequencePara()
{
    return m_Frequence;
}

void SquModel::SetWaveAmp(Para val)
{
    m_Amp=val;
    return;
}

Para SquModel::GetWaveAmp()
{
    return m_Amp;
}
...
{
    DWORD writeSize;
    uint8_t Data[8]={0xAB,0x01,0x26,0x00,0x00,0x00,0x01,0xBA};
